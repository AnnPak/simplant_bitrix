					...
					<div class="row">
						<div class="col-md-12">
							<?if($arParams['TITLE_BLOCK'] || $arParams['TITLE_BLOCK_ALL']):?>
								<h3 class="title_block reviews-block_title"><?=$arParams['TITLE_BLOCK'];?></h3>
								<?// intro text?>
								<?if($arParams['INCLUDE_FILE']):?>
								...
								<div class="block-links">							
									<?if($arParams['SHOW_ADD_REVIEW'] == 'Y' && $arParams['TITLE_ADD_REVIEW'] ):?>
										<input type="button" class="btn btn-transparent-border-color animate-load btn-reviews_block" data-event="jqm" data-param-form_id="REVIEW" data-name="send_review" value="<?=$arParams['TITLE_ADD_REVIEW']?>"><?=CNext::showIconSvg("resume colored", SITE_TEMPLATE_PATH."/images/svg/leaveareview.svg", "", "", true, false)?>
									<?endif;?>
								</div>
							<?endif;?>
						</div>
				...
				<ul class="items slides flexbox row">
					<?foreach($arResult['ITEMS'] as $i => $arItem):?>
						...
						<li class="item-wrapper col-xs-12<?=(!$bOneItem ?  ' bg-fill-white bordered1 box-shadow1' : '');?>">
							<div class="item reviews-block_item clearfix <?=($bLogo ? ' wlogo' : '')?> <?=(!$bImage ? 'no_img' : '')?> <?=(!$bOneItem ? ' bordered box-shadow' : '');?> <?=($notSlider ? ' rounded2 bordered' : '');?>" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
								<div class="top_wrapper clearfix">
									<?if($imageSrc):?>
										<div class="pull-left reviews-block_avatar">
											<div class="wrap">
									...
									<?endif;?>
									<div class="top-info">
										<div class="reviews-block_date muted">
										...
										<div class="reviews-block_name <?=($bCompact ? 'font_md' : 'font_lg');?> pull-left1">
											<?if($bDetailLink):?><a href="<?=$arItem['DETAIL_PAGE_URL']?>"><?endif;?>
											<?=$arItem['NAME'];?>
											<?if($bDetailLink):?></a><?endif;?>
										</div>
									...
									<div class="body-info clearfix">
										...
										<?if(($arParams['PREVIEW_TRUNCATE_LEN'] > 0) && (strlen($arItem['~PREVIEW_TEXT']) > $arParams['PREVIEW_TRUNCATE_LEN'])):?>
											<div class="link-block-more">
												<span><span class="btn btn_show-more-reviews dyn-jsform" data-event="jqm" data-param-form_id="<?=$arItem['ID'];?>" data-param-type="review" data-name="review-detail"><?=Loc::getMessage('MORE_REVIEWS');?></span></span>
											</div>
										<?endif;?>
